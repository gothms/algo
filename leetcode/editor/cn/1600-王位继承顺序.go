...
//
//
// Related Topics 树 深度优先搜索 设计 哈希表 👍 76 👎 0
...
func Constructor(kingName string) ThroneInheritance {
	return ThroneInheritance{kingName, make(map[string][]string), make(map[string]struct{})}
}
...
func (this *ThroneInheritance) GetInheritanceOrder() []string {
	...
	var dfs func(string)
	dfs = func(c string) {
		if _, ok := this.death[c]; !ok { // 添加
			ret = append(ret, c)
		}
		for _, s := range this.nodes[c] {
			dfs(s)
		}
	}
	dfs(this.cur)
