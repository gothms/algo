...
//
//
// Related Topics 栈 数组 双指针 二分查找 单调栈 👍 72 👎 0
...
func maximumScore(nums []int, k int) int {
	// lc：双指针
	ret, n := nums[k], len(nums)
	for l, r, v := k-1, k+1, ret; l >= 0 || r < n; {
		for l >= 0 && nums[l] >= v {
			l--
		}
		for r < n && nums[r] >= v {
		...
		ret = max(ret, v*(r-l-1)) // 区间 (l, r) 的最小值为 v
		var lv, rv int            // 越界判断
		if l >= 0 {
			lv = nums[l]
		}
	...
	// 单调栈
	//ret, n, stack := 0, len(nums), []int{-1} // 单调递增栈
	//for i, v := range nums {
	//	last := len(stack) - 1
	//	for last > 0 && v < nums[stack[last]] {
	//		last--
	//		if i > k && stack[last] < k { // 区间 (stack[last], i) 的最小值为 nums[stack[last+1]]
	//			ret = max(ret, nums[stack[last+1]]*(i-stack[last]-1))
	//		}
	//	}
	//	stack = append(stack[:last+1], i)
	//}
	//idx := sort.SearchInts(stack, k) // 剩下的好子数组为：(stack[i-1], n)，最小值为 nums[stack[i]]
	//for i := 1; i <= idx; i++ {
	//	ret = max(ret, nums[stack[i]]*(n-stack[i-1]-1))
	//}
	//return ret
}
