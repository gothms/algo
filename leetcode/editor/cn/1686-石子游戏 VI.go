...
//
//
// Related Topics 贪心 数组 数学 博弈 排序 堆（优先队列） 👍 65 👎 0
...
func stoneGameVI(aliceValues []int, bobValues []int) int {
	// 贪心
	sa, sb, n := 0, 0, len(aliceValues)
	memo := make([][2]int, n)
	for i := 0; i < n; i++ {
		memo[i] = [2]int{i, aliceValues[i] + bobValues[i]}
	}
	sort.Slice(memo, func(i, j int) bool {
		return memo[i][1] > memo[j][1]
	})
	for i := 0; i < n; i++ {
		idx := memo[i][0]
		if i&1 == 0 {
