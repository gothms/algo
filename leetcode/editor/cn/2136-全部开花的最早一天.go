...
func earliestFullBloom(plantTime []int, growTime []int) int {
	...
	efb, plant, n := 0, 0, len(plantTime)
	id := make([]int, n)
	for i := 1; i < n; i++ { // 索引
		id[i] = i
	}
	sort.Slice(id, func(i, j int) bool {
		return growTime[id[i]] > growTime[id[j]] // 贪心：优先种生长时间长的花
		//return growTime[id[i]] > growTime[id[j]] ||
		//	growTime[id[i]] == growTime[id[j]] && plantTime[id[i]] > plantTime[id[j]]
	})
	for _, i := range id {
		plant += plantTime[i]                // 种花总耗时
		efb = maxVal(efb, plant+growTime[i]) // 这朵花开的时间
	}
	return efb
}
