...
func main() {
	...
	version := compareVersion(version1, version2)
	fmt.Println(version)
}
...
func compareVersion(version1 string, version2 string) int {
	...
	// 迭代
	v1, v2, n, m := 0, 0, len(version1), len(version2)
	for i, j, idx, jdx := 0, 0, 0, 0; i < n || j < m; {
		v1, v2 = 0, 0
		if i < n {
			for idx = i + 1; idx < n && version1[idx] != '.'; {
				idx++
			}
			v1, _ = strconv.Atoi(version1[i:idx])
			i = idx + 1
		}
		if j < m {
			for jdx = j + 1; jdx < m && version2[jdx] != '.'; {
				jdx++
			}
			v2, _ = strconv.Atoi(version2[j:jdx])
			j = jdx + 1
		}
		if v1 > v2 {
			return 1
