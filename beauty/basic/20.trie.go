...
Trie 树真的很耗内存吗？
	...
	Trie 树的本质是避免重复存储一组字符串的相同前缀子串，但是现在每个字符（对应一个节点）的存储远远大于 1 个字节
		...
		每个数组元素要存储一个 8 字节指针（或者是 4 字节，这个大小跟 CPU、操作系统、编译器等有关）
			数组长度为 26，每个元素是 8 字节，那每个节点就会额外需要 26*8=208 个字节
			如果字符串中不仅包含小写字母，还包含大写字母、数字、甚至是中文，那需要的存储空间就更多了
		在某些情况下，Trie 树不一定会节省存储空间
...
如何利用 Trie 树，实现搜索关键词的提示功能？
	...
	当用户输入其中某个单词的时候，把这个词作为一个前缀子串在 Trie 树中匹配，并展示在搜索提示框内
	这就是搜索关键词提示的最基本的算法原理
深入问题
...
思考
	...
	// TODO
		...
		前缀重合程度
			假设全是小写英文字母
			第一遍，将首字符相同的字符串放入同一个集合中（一共26个集合）
			第二遍，再分别处理26个集合中的字符串的第二个字符，同第一遍
			...
			每一遍都可以计算，当前集合的大小与上一级集合的大小的比值关系
*/
