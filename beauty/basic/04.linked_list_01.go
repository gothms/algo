...
缓存
	一种提高数据读取性能的技术
		在硬件设计、软件开发中都有着非常广泛的应用
		比如常见的 CPU 缓存、数据库缓存、浏览器缓存等
	常见的缓存淘汰策略有三种
		先进先出策略 FIFO（First In，First Out）
		最少使用策略 LFU（Least Frequently Used）：E:\gothmslee\algo\database\lfu.go
		最近最少使用策略 LRU（Least Recently Used）：E:\gothmslee\algo\database\lru.go
	利用了空间换时间的设计思想
		把数据存储在硬盘上，会比较节省内存，但每次查找数据都要询问一次硬盘，会比较慢
		事先将数据加载在内存中，虽然会比较耗费内存空间，但是每次数据查询的速度就大大提高了
...
三种最常见的链表结构
	...
	双向链表
	循环链表
单链表：头结点用来记录链表的基地址，以此遍历得到整条链表，而尾结点指向一个空地址 NULL
...
单链表 VS 双向链表：空间换时间
	...
	查询有序链表
		双向链表可以往前按值查询
		根据要查找的值与当前结点的大小关系，决定是往前还是往后查找，所以平均只需要查找一半的数据
双向循环链表
...
时间 VS 空间
	对于执行较慢的程序，可以通过消耗更多的内存（空间换时间）来进行优化
		当内存空间充足的时候，如果更加追求代码的执行速度
		可以选择空间复杂度相对较高、但时间复杂度相对很低的算法或者数据结构
		缓存
	而消耗过多内存的程序，可以通过消耗更多的时间（时间换空间）来降低内存的消耗
		如果内存比较紧缺，比如代码跑在手机或者单片机上
...
思考
	利用数组实现 LRU 缓存淘汰策略
		E:\gothmslee\algo\database\lru.go
