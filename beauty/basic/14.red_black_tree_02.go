...
为什么红黑树的定义中，要求叶子节点是黑色的空节点？
	只要满足这一条要求，那在任何时刻，红黑树的平衡操作都可以归结为上面的那几种情况
		任意节点到叶子节点的所有路径含等量黑色节点
		插入删除前后，通过规则实现保证了前后规则的一致性，也保证了红黑树的平衡性
...
实现红黑树的总结
	...
	找准关注节点，不要搞丢、搞错关注节点
		每种操作规则，都是基于关注节点来做的，只有弄对了关注节点，才能对应到正确的操作规则中
	插入操作的平衡调整比较简单，但是删除操作就比较复杂
		两次调整
		第一次是针对要删除的节点做初步调整，让调整后的红黑树继续满足第四条定义，“每个节点到可达叶子节点的路径都包含相同个数的黑色节点”
			此时，第三条定义就不满足了，有可能会存在两个红色节点相邻的情况
