...
LRU 缓存淘汰算法
	...
	双向链表+散列表
		...
		插入：O(1)
	hnext
		pre 和 next 连接的双向链表是按照缓存的时间由大到小，组成的一个缓存队列
		hnext 作用是，在最新时间插入缓存数据时通过哈希函数得出的散列冲突，用其连接
		图解：11_hash_table_lru.jpg
Redis 有序集合
	细化一下 Redis 有序集合的操作
		...
		按照分值从小到大排序成员变量
	key & score
		score：仅仅按照分值将成员对象组织成跳表的结构，那按照键值来删除、查询成员对象就会很慢
		key：按照键值构建一个散列表，样按照 key 来删除、查找一个成员对象的时间复杂度就变成了 O(1)
