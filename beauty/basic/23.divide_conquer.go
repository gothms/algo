...
如何给 10GB 的订单文件按照金额排序，内存可能只有 2、3GB？
	...
	GFS：在“桶”的基础上
		...
		注意：
			有一个点要注意，就是数据的存储与计算所在的机器是同一个或者在网络中靠的很近（比如一个局域网内，数据存取速度很快）
...
两个经典问题
	二维平面上有 n 个点，如何快速计算出两个距离最近的点对？
		...
		排序：按照 x 为第一关键字、y 为第二关键字排序
		分治：以中点 m 为分界点，拆分点集
		边界条件：子集点个数 <=2
		归并：两个子集所得的较小距离为 h
			a.所有横坐标与 xm 的差 <h 的点放入一个集合 B
			b.将 B 中的点按照 y 排序（可改变策略优化到 O(n)）
			c.对于每个 B 中的点 p，考虑其 y 坐标范围 (y-h,y+h) 内的其他点，计算距离并更新答案
		补充：
			两次排序
				第一次：只在分治开始前进行一次
				第二次：令每次递归返回当前点集按 y 排序的结果，对于第二次排序，上层直接使用下层的两个分别排序过的点集归并即可。O(n)
			复杂度证明
				归并时，点 p 的 (y-h,y+h) 范围最大大小为 7，包含 p 自身
				详见 oi-wiki
			推广：平面最小周长三角形
				在给定的一组点中，选择三个点，使得它们两两的距离之和最小
				提示：3个点周长为 d，建立“中间”集合的条件为 d/2（最大边 <d/2）
		非分治算法
			...
		期望线性做法
			...
		实现：E:\gothmslee\algo\divide\nearestPoints.go
	有两个 n*n 的矩阵 A，B，如何快速求解两个矩阵的乘积 C=A*B？
		https://oi-wiki.org/math/linear-algebra/matrix/
		算法导论，dp 那一节有讲到矩阵乘法
		实现：E:\gothmslee\algo\divide\matrixMultiply.go
思考
	basic 中的数据结构、算法、解决思路，以及举的例子中，有哪些采用了分治算法的思想呢？
		1.快速排序算法
		2.归排序算法
		3.桶（计数）排序算法
		4.散列表的应用（负载均衡、分片、分布式存储）
		5.利用递归树求解算法复杂度的思想
		6.MapReduce 模型处理思想
