...
1.如何实现精确匹配规则？
	...
	匹配规则：
		...
		字符串数组：可以将每个应用对应的权限规则，存储在一个字符串数组中
...
2.如何实现前缀匹配规则？
	...
	匹配规则：
		...
		Trie树：非常适合用来做前缀匹配，存储每个应用对应的权限规则
			...
			因为规则并不会经常变动
			所以，在 Trie 树中，可以把每个节点的子节点们，组织成有序数组这种数据结构
			在匹配的过程中，可以利用二分查找算法，决定从一个节点应该跳到哪一个子节点
			04.micro_service_route_match-2-binary.jpg
3.如何实现模糊匹配规则？
	...
	滑动时间窗口限流算法
		...
		实现
			...
			那就维护一个大小为 K+1 的循环队列，用来记录 1s 内到来的请求
				循环队列的大小等于限流次数加一，因为循环队列存储数据时会浪费一个存储单元
		...
		缺点
			...
			比如第一个 1s 的时间窗口内，100 次请求都集中在最后 10ms 中
	小结
...
思考
	1.除了用循环队列来实现滑动时间窗口限流算法之外，是否还可以用其他数据结构来实现呢？请对比一下这些数据结构跟循环队列在解决这个问题时的优劣之处
