...
DFS 算法
	...
	时间复杂度：O(V+E)（V 表示顶点个数，E 表示边的个数）
代码实现
	E:\gothmslee\algo\algo\topological.go
...
总结
	...
	拓扑排序能检测图中环的存在
		...
		案例：basic 07.recursive.go，递归查找最终推荐人的时候，可能会因为脏数据，造成存在循环推荐
			...
			示例：func findRootReferrerId(actorId int) int
...
思考
	...
	Kahn 算法和 DFS 算法还能否正确工作呢？如果不能，应该如何改造一下呢？
		1).Kahn：逆邻接表的第二维数组长度为 0，则代表 入度==0
			但是，既然长度为 0，就没法根据 入度=0 的顶点在逆邻接表中查找“下一个”顶点了
			将逆邻接表改为邻接表即可
		2).DFS：仍然能正确工作，只是拓扑排序的结果要反过来
	2.两种拓扑排序算法的实现思路，Kahn 算法和 DFS 深度优先搜索算法，如果换做 BFS 广度优先搜索算法，还可以实现吗？
		Kahn vs BFS
		Kahn：不是逐层遍历，入队列的条件是 入度==0
		BFS：遍历到的顶点，立刻就加入队列
*/

// findRootReferrerId 记录已经访问过的用户 ID，当用户 ID 第二次被访问的时候，就说明存在环
//func findRootReferrerId(actorId int) int {
//	memo := make(map[int]struct{})
//	if _, ok := memo[actorId]; ok { // 存在环
//		return -1
//	}
//	memo[actorId] = struct{}{}
