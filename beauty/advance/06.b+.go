...
改造二叉查找树来解决这个问题
	...
	改造过程：3
		...
		计算m：多大才最合适呢？
			...
			页：
				不管是内存中的数据，还是磁盘中的数据，操作系统都是按页（一页大小通常是 4KB）来读取的，一次会读一页的数据
...
思考：参见留言区
	...
	散列表也经常跟链表一块使用，如果我们把散列表中的结点，也用链表串起来，能否支持按照区间查找数据呢？
		LinkedHashMap 保证不了按区间排序，只是保存放入的先后次序，并不能保证大小的顺序关系
		想要按区间排序，需要额外对散列表中的节点再加一层双向链表的关系，用来维护节点的大小关系
		但是此时的插入操作的时间复杂度会比较高。每次插入一个元素，需要遍历并比较现有的节点，找到合适的位置来建立链表关系，时间复杂度为O(n)
		LinkedHashMap 参考 E:\gothmslee\algo\beauty\basic\11.hash_table_03.go
*/
