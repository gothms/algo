...
算法解析
	初步分析
		...
		实际上，像出行路线规划、游戏寻路，这些真实软件开发中的问题，一般情况下，我们都不需要非得求最优解（也就是最短路径）
	...
	如何快速找出一条接近于最短路线的次优路线呢？
		...
		算法优化：启发函数 & 曼哈顿距离
			当我们遍历到某个顶点的时候，从起点走到这个顶点的路径长度是确定的，我们记作 g(i)（i 表示顶点编号）
			但是，从这个顶点到终点的路径长度，是未知的。虽然确切的值无法提前知道，但是可以用其他估计值来代替
			通过这个顶点跟终点之间的直线距离，也就是欧几里得距离，来近似地估计这个顶点跟终点的路径长度（注意：路径长度跟直线距离是两个概念）
			把这个距离记作 h(i)（i 表示这个顶点的编号），专业的叫法是启发函数（heuristic function）
			因为欧几里得距离的计算公式，会涉及比较耗时的开根号计算，一般通过另外一个更加简单的距离计算公式，那就是曼哈顿距离（Manhattan distance）
		...
		估价函数（evaluation function）
			...
			g(i)：顶点与起点之间的路径长度 g(i)，Dijkstra 算法判断谁先出队列
			h(i)：启发函数（曼哈顿距离）
	...
	实现：A* vs Dijkstra
		...
		主要逻辑区别
			...
			3.循环结束的条件也不一样
				...
				A* 算法是一旦遍历到终点就结束
		E:\gothmslee\algo\algo\astar.go
	尽管 A* 算法可以更加快速地找到从起点到终点的路线，但是它并不能像 Dijkstra 算法那样，找到最短路线。这是为什么呢？
		Dijkstra
			回溯：07.a_search_dijkstra-back.jpg
				要找出起点 s 到终点 t 的最短路径，最简单的方法是
				通过回溯穷举所有从 s 到达 t 的不同路径，然后对比找出最短的那个
			动态规划思想：07.a_search_dijkstra-dp.jpg
				Dijkstra 算法在此基础之上，利用动态规划的思想，对回溯搜索进行了剪枝，只保留起点到某个顶点的最短路径，继续往外扩展搜索
...
如何借助 A* 算法解决游戏寻路问题？
	...
	2.边权值为 1 的有向图
		可以把每个方块看作一个顶点
		两个方块相邻，我们就在它们之间，连两条有向边，并且边的权值都是 1
		问题就转化成了，在一个有向有权图中，找某个顶点到另一个顶点的路径问题
		将地图抽象成边权值为 1 的有向图之后，我们就可以套用 A* 算法，来实现游戏中人物的自动寻路功能了
...
总结
	...
	工程中
		...
		比如，地图 App 中的出行路线规划问题，不仅可以使用 Dijkstra 算法，也可以利用启发式搜索算法来实现

思考
	“迷宫问题”是否可以借助 A* 算法来更快速地找到一个走出去的路线呢？如果可以，请具体讲讲该怎么来做；如果不可以，请说说原因
		因为 A* 每次都找 f 值最小的顶点出队列，优先选择的是离出口更近的路径
*/
