...
位图（BitMap）
	...
	位图方案：特殊的散列表
		...
		实现
			E:\gothmslee\algo\algo\bitmap.go
	内存空间对比
		散列表
...
布隆过滤器（Bloom Filter）
	...
	误判：布隆过滤器的误判有一个特点，那就是，它只会对存在的情况有误判
...
执行效率比较
	...
	布隆过滤器：CPU 密集型
		用多个哈希函数对同一个网页链接进行处理，CPU 只需要将网页链接从内存中读取一次，进行多次哈希计算
		理论上讲这组操作是 CPU 密集型的
	CPU 计算比内存访问更快速
...
总结
	...
	自动扩容
		往布隆过滤器中不停地加入数据之后，位图中不是 true 的位置就越来越少了，误判率就越来越高了
	...
	布隆过滤器本身就是解决位图消耗空间比较多的问题
		位图的大小是数据的范围
		而布隆过滤器的大小应该是小于位图大小的

思考
	1.假设有 1 亿个整数，数据范围是从 1 到 10 亿，如何快速并且省内存地给这 1 亿个数据从小到大排序？
		呼应 E:\gothmslee\algo\beauty\basic\08.sort_03.go
		位图
		散列表记录超过1个的重复数字的个数
	2.basic.12.hash_02.go 中的：数据分片 示例二、如何快速判断图片是否在图库中？利用分治思想，用散列表以及哈希函数，实现海量图库中的判重功能
	如果允许小概率的误判，那是否可以用布隆过滤器来解决呢？请参照当时的估算方法，重新估算下，用布隆过滤器需要多少台机器？
		用散列表和哈希函数的方法时，存了图片的地址，先哈希值比再全量比。如果允许错判，那可以不存储地址了，只比哈希值
		// TODO
*/
