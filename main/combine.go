...
func main() {
	...
	dfs := maths.SubsetsWithDup(nums)
	fmt.Println(dfs, len(dfs))
	dfs = maths.SubsetsWithDupDFS(nums)
	...
	counter := maths.SubsetsAndCombineCounter(nums)
	...
	k := 3
	k = 2
	dup := maths.CombineWithDup(nums, k)
	//fmt.Println(dup, len(dup)) // 2：17，3：31，4：41，5：41，6：31，7：17
	dup = maths.CombineWithDupDFS(nums, k)
	fmt.Println(dup, len(dup)) // 2：17，3：31，4：41，5：41，6：31，7：17
	counterK := maths.SubsetsAndCombineCounterK(nums, k)
	fmt.Println(counterK)

	// 子集 & 排列
	nums = []int{1, 2, 3, 3, 1}
	andPermute := maths.SubsetsAndPermute(nums)
	fmt.Println(nums)
	//fmt.Println(andPermute, len(andPermute))
	for _, ap := range andPermute {
		fmt.Println("SubsetsAndPermute：", ap, len(ap))
	}
	permuteCounter := maths.SubsetsAndPermuteCounter(nums)
	...
	nums = []int{1, 2, 3}
	k = 3
	permuteK := maths.PermuteK(nums, k)
	fmt.Println("permuteK：", permuteK, len(permuteK))
	permuteK = maths.PermuteAll(nums)
	...
	permuteK = maths.SubsetsAndPermute(nums)
	fmt.Println("SubsetsAndPermute：", len(permuteK))
	nums = []int{1, 3, 4, 8, 10, 11}
	permuteK = maths.PermuteUnique(nums)
	...
	permuteK = maths.PermuteUniqueK(nums, k)
